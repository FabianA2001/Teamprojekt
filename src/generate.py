...
def generate_areas(count: int, height: int, width: int) -> list[Polygon]:
    """
    ...
    for _ in range(count):
        for attempt in range(100):
            area_location = random_coord_global(height, width)
            for location in areas_list:
                ...
                if is_enough_distance(area_location, location, CONST.MIN_DISTANCE_POLYGONS):
                    verifier = True
                else:
                    verifier = False
                    break
